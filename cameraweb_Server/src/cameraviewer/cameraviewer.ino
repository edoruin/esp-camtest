...
void conectarMQTT() {
  while (!mqtt.connected()) {
    ...
    if (mqtt.connect(clientId, MQTT_USER, MQTT_PASS)) {
      ...
      char payload[64];
      snprintf(payload, sizeof(payload), "{\"ip\":\"%s\"}", ip.c_str());
      
      // Publicar con retain en true
      if (mqtt.publish(TOPIC_CAMERA_IP, payload, true)) {
        Serial.printf("[MQTT] IP publicada: %s\n", payload);
      } else {
        Serial.println("[MQTT] Fallo al publicar IP.");
      }
    } else {
      int rc = mqtt.state();
...
void setup() {
  ...
  // Generar Client ID único y configurar MQTT
  snprintf(clientId, sizeof(clientId), "ESP32_CAM_%04X", (uint16_t)random(0xFFFF));
